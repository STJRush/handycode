...
while True:
    ...
    try:
        ...
        humidDHT, tempDHT = DHT.read_retry(DHT.DHT11, 4) #On GPIO Pin 4
        print(humidDHT, tempDHT)
        ...
        if humidDHT != None and tempDHT != None:
            f = urlopen(baseURL + "&field3=%s" % (ccs.geteCO2()) + "&field4=%s" % (ccs.getTVOC())+ "&field5=%s" % (tempDHT)) 
            print (f.read()) 
            f.close() 
            sleep(300)
        else:
            print("Didn't work due to DHT, tyring again now")
            sleep(5)
