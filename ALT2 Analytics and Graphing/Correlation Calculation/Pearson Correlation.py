...
# In this example, it looks at data from 4 geiger counters namely QA060,Bella,FS2011,XH-901
...
print(f"Correlation between QA060 and Bella: {corr_QA060_Bella} ({interpret_correlation(corr_QA060_Bella)})")
...
print(f"Correlation between Bella and FS2011: {corr_Bella_FS2011} ({interpret_correlation(corr_Bella_FS2011)})")
print(f"Correlation between Bella and XH-901: {corr_Bella_XH901} ({interpret_correlation(corr_Bella_XH901)})")
...
# Assuming the columns are named 'QA060', 'Bella', 'FS2011', 'XH-901'
plt.plot(data['QA060'], label='QA060')
plt.plot(data['Bella'], label='Bella')
plt.plot(data['FS2011'], label='FS2011')
plt.plot(data['XH-901'], label='XH-901')

# Adding title and labels
plt.title('Line Graph of QA060, Bella, FS2011, XH-901')
plt.xlabel('Index')
plt.ylabel('Value')
...
corr_qa060_bella, _ = spearmanr(qa060, bella)
corr_qa060_fs2011, _ = spearmanr(qa060, fs2011)
corr_qa060_xh901, _ = spearmanr(qa060, xh901)
corr_bella_fs2011, _ = spearmanr(bella, fs2011)
corr_bella_xh901, _ = spearmanr(bella, xh901)
...
print(f"Spearman's correlation between QA060 and Bella: {corr_qa060_bella} ({interpret_correlation(corr_QA060_Bella)})")
...
print(f"Spearman's correlation between Bella and FS2011: {corr_bella_fs2011} ({interpret_correlation(corr_Bella_FS2011)})")
print(f"Spearman's correlation between Bella and XH-901: {corr_bella_xh901} ({interpret_correlation(corr_Bella_XH901)})")
